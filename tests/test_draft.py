...
from mailcore.types import EmailAddress, SendResult
...
async def test_send_with_quote_fetches_body(mock_smtp, mock_message):
    ...
    draft = Draft(smtp=mock_smtp, reference_message=mock_message, quote=True)
...
async def test_send_with_attachments_fetches_content(mock_smtp, mock_message):
    ...
    draft = Draft(smtp=mock_smtp, reference_message=mock_message, include_attachments=True)
